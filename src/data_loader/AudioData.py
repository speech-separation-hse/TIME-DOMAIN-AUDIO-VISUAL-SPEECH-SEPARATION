...
def read_wav(fname, return_rate=False, num_frames=0, offset=0):
    '''
         ...
         output:
                ...
                sr: sample rate
    '''
    src, sr = torchaudio.load(fname, channels_first=True, num_frames=num_frames, offset=offset)
    if return_rate:
...
class AudioReader(object):
    '''
        ...
        Input:
            audio_csv: file path of all audio (type: str)
    ...
    def __init__(self, audio_csv, sample_rate=8000, chunk_size=32000, least_size=16000):
        super(AudioReader, self).__init__()
        self.sample_rate = sample_rate
        # self.index_dict = util.handle_scp(scp_path)
        # self.keys = list(self.index_dict.keys())
        self.audios_info = pd.read_csv(audio_csv)
        self.audio = []
